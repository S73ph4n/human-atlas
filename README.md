...
- Switch to CT radioanatomy: scroll through real labelled CT studies of the chest–pelvis and the head and neck in three planes, with soft-tissue, lung, bone, and brain windows.
...
The CT view uses two contrast-enhanced studies from the **TotalSegmentator dataset v2.0.1**, licensed **CC BY 4.0**, both with 1.5 mm voxels: chest to pelvis (subject s0476, 108 labelled structures, about 16 MB) and head and neck (subject s0777, 115 labelled structures, 68 of them generated by the TotalSegmentator model and marked as unverified, about 6.5 MB). Each study downloads only when it is opened.
...
Download a subject folder from the [TotalSegmentator dataset](https://zenodo.org/records/10047292) (it contains `ct.nii.gz` and `segmentations/`), install `nibabel`, `numpy`, and `scipy`, then run `python scripts/convert-ct.py <subject folder>` (add `--bits 16` for studies read with narrow windows, and `--generated <folder> …` to merge masks predicted by TotalSegmentator subtasks). The output goes to `public/ct/<subject>/`; add the study to `CT_STUDIES` in `app/ct.ts`.
